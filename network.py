...
class ConvNet(nn.Module):
    def __init__(self, grid_size):
        ...
        self.conv2 = nn.Conv2d(in_channels=20, out_channels=1, kernel_size=3, stride=1)
        self.relu = nn.ReLU()
        self.softm = nn.Softmax(dim=-1)

    def forward(self, x):
        x = F.pad(x, (1, 1, 1, 1), 'constant', 1)  # Pad with ones
        # x = F.pad(x, (1, 1, 1, 1), 'constant', 1)  # Pad with ones

        x = self.conv1(x)
        x = self.relu(x)
        x = F.pad(x, (1, 1, 1, 1), 'constant', 1)  # Pad with ones again for the next conv layer
        # x = F.pad(x, (1, 1, 1, 1), 'constant', 1)  # Pad with ones again for the next conv layer
...
class ConvVal(nn.Module):
    def __init__(self, grid_size):
        ...
        self.conv2 = nn.Conv2d(in_channels=10, out_channels=1, kernel_size=3, stride=1)
        self.relu = nn.ReLU()
