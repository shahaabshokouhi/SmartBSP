...
actor1.load_state_dict(torch.load('ppo_actor.pth'))
actor2 = ConvNet(grid_size=n)
actor2.load_state_dict(torch.load('ppo_actor1.pth'))
batch_rew_history = []
obs_cols = []
render = True
...
for idx, batch_grids in enumerate(gridloader):
    ...
    dist_map1 = actor1(batch_grids)
    dist_map_numpy1 = dist_map1.detach().numpy()

    dist_map2 = actor2(batch_grids)
    dist_map_numpy2 = dist_map2.detach().numpy()

    for j in range(dist_map_numpy1.shape[0]):
        plan = 1
        actions = []
        for i in range(n-1):
            action = np.argmax(dist_map_numpy1[j, :, i+1], axis=0)
            actions.append(action)
        actions = np.array(actions)
        y = path_planner.action2point(actions)
        p = np.column_stack((x, y))
        path = path_planner.construct_sp(p)
        if path_planner.obstacle_check(batch_grids[j, 0]):
            plan = 2
            actions = []
            for i in range(n-1):
                action = np.argmax(dist_map_numpy2[j, :, i+1], axis=0)
                actions.append(action)
            actions = np.array(actions)
            y = path_planner.action2point(actions)
            p = np.column_stack((x, y))
        ...
        if obs_col and render:
            ...
            if plan == 1:
                dist_map_numpy_grid = dist_map_numpy1[j]
            else:
                dist_map_numpy_grid = dist_map_numpy2[j]
